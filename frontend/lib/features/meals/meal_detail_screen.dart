          ...
          children: [
            ...
            for (final it in m.items)
              ListTile(
                ...
                subtitle: it.tags.isEmpty ? null : Wrap(spacing: 4, children: [for (final t in it.tags) Chip(label: Text(t))]),
                trailing: it.quantityG == null ? null : Text('${it.quantityG!.toStringAsFixed(0)} g'),
              ),
            if (m.notes != null) ...[
              const SizedBox(height: 16),
              Text(t.mealFieldNotes, style: Theme.of(context).textTheme.titleMedium),
              Text(m.notes!),
            ],
          ],
        ),
